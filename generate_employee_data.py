...
for i in range(start_range, end_range):  # Start from 1 and go up to 1,000,000
    ...
    entries.append((name, last_name, job_title, manager, hire_date, salary))
...
for i in range(0, len(entries), batch_size):
    ...
    values = []
    for entry in batch:
        manager_value = 'NULL' if entry[3] is None else entry[3]
        values.append(f"('{entry[0]}', '{entry[1]}', '{entry[2]}', {manager_value}, '{entry[4]}', {entry[5]})")
    
    sql_insert_statements.append(
        "INSERT INTO Employees (Name, LastName, JobTitle, Manager, HireDate, Salary) VALUES\n" +
        ",\n".join(values) + ";"
    )
